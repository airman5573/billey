...
class Widget_Heading extends Base {
	...
	public function get_title() {
		return esc_html__( 'Modern Heading', 'billey' );
	}
	...
	private function add_title_section() {
		$this->start_controls_section( 'title_section', [
			'label' => esc_html__( 'Heading', 'billey' ),
		] );
		...
		$this->add_control( 'title', [
			...
			'description' => esc_html__( 'Wrap any words with &lt;mark&gt;&lt;/mark&gt; tag to make them highlight.', 'billey' ),
		] );
		...
		$this->add_control( 'title_link_animate', [
			'label'        => esc_html__( 'Link Animate', 'billey' ),
			'type'         => Controls_Manager::SELECT,
			'options'      => [
				''                  => esc_html__( 'None', 'billey' ),
				'animate-border'    => esc_html__( 'Animate Border', 'billey' ),
				'animate-border-02' => esc_html__( 'Animate Border 02', 'billey' ),
			],
			'default'      => '',
			'prefix_class' => 'billey-link-',
		] );
	...
	private function add_sub_title_section() {
		$this->start_controls_section( 'sub_title_section', [
			'label' => esc_html__( 'Secondary Heading', 'billey' ),
		] );
	...
	private function add_title_style_section() {
		...
		$this->add_responsive_control( 'title_margin', [
			...
			'size_units' => [ 'px', '%', 'em' ],
			'selectors'  => [
				'{{WRAPPER}} .heading-primary-wrap' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		] );
		...
		$this->add_control( 'blend_mode', [
			'label'     => esc_html__( 'Blend Mode', 'billey' ),
			'type'      => Controls_Manager::SELECT,
			'options'   => [
		...
		$this->add_control( 'highlight_heading', [
			'label'     => esc_html__( 'Highlight Words', 'billey' ),
		...
		$this->add_responsive_control( 'title_animate_line_height', [
			...
			'selectors'  => [
				'{{WRAPPER}} .heading-primary a:before, {{WRAPPER}} .heading-primary a:after' => 'height: {{SIZE}}{{UNIT}};',
			],
			'condition'  => $line_condition,
		] );
		...
		$this->add_control( 'title_animate_line_color', [
			...
			'selectors' => [
				'{{WRAPPER}} .heading-primary a:before, {{WRAPPER}} .heading-primary a:after' => 'background: {{VALUE}};',
			],
			'condition' => $line_condition,
		] );
		...
		$this->add_control( 'hover_title_animate_line_color', [
			...
			'type'      => Controls_Manager::COLOR,
			'selectors' => [
				'{{WRAPPER}} .heading-primary a:after' => 'background: {{VALUE}};',
			],
			'condition' => [
				'title_link_animate' => [
					'animate-border',
	...
	private function add_divider_style_section() {
		$this->start_controls_section( 'divider_style_section', [
			'label'     => esc_html__( 'Divider', 'billey' ),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
				'divider_enable' => 'yes',
			],
	...
	private function add_sub_title_style_section() {
		...
		$this->add_control( 'sub_title_blend_mode', [
			'label'     => esc_html__( 'Blend Mode', 'billey' ),
			'type'      => Controls_Manager::SELECT,
			'options'   => [
				''            => esc_html__( 'Normal', 'billey' ),
			...
			'selectors' => [
				'{{WRAPPER}} .heading-secondary' => 'mix-blend-mode: {{VALUE}}',
			],
			'separator' => 'none',
		] );
	...
	private function print_description(array $settings) {
		...
		<div class="heading-description-wrap">
			<div <?php $this->print_render_attribute_string( 'description' ); ?>>
				<?php echo wp_kses_post($settings['description']); // WYSIWYG editor. ?>
			</div>
		</div>
	<?php
    }
