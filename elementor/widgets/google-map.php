...
class Widget_Google_Map extends Base {
	...
	public function get_title() {
		return esc_html__( 'Advanced Google Map', 'billey' );
	}
	...
	private function add_map_options_section() {
		...
		$this->add_control( 'style', [
			'label'   => esc_html__( 'Map Style', 'billey' ),
		...
		$this->add_control( 'type', [
			'label'   => esc_html__( 'Map Type', 'billey' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'roadmap'   => esc_html__( 'Roadmap', 'billey' ),
				'satellite' => esc_html__( 'Satellite', 'billey' ),
				'hybrid'    => esc_html__( 'Hybrid', 'billey' ),
				'terrain'   => esc_html__( 'Terrain', 'billey' ),
			],
			'default' => 'roadmap',
		] );

		$this->add_control( 'zoom_level', [
			'label'       => esc_html__( 'Zoom Level', 'billey' ),
			'placeholder' => '12',
		...
		$this->add_control( 'scrollwheel', [
			'label'   => esc_html__( 'Mouse Scroll Wheel Zoom', 'billey' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );

		$this->add_control( 'draggable', [
			'label'   => esc_html__( 'Draggable', 'billey' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );

		$this->add_control( 'map_controls_heading', [
			'label'     => esc_html__( 'Controls', 'billey' ),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		] );

		$this->add_control( 'zoom_control', [
			'label' => esc_html__( 'Zoom Control', 'billey' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'scale_control', [
			'label' => esc_html__( 'Scale Control', 'billey' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'fullscreen_control', [
			'label' => esc_html__( 'Fullscreen Control', 'billey' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'street_view_control', [
			'label' => esc_html__( 'Street View Control', 'billey' ),
			'type'  => Controls_Manager::SWITCHER,
		] );
	...
	private function add_map_markers_section() {
		$this->start_controls_section( 'markers_section', [
			'label' => esc_html__( 'Markers', 'billey' ),
		] );

		$this->add_control( 'marker_style', [
			'label'       => esc_html__( 'Marker Style', 'billey' ),
			'description' => esc_html__( 'Select default style for all markers', 'billey' ),
			'type'        => Controls_Manager::SELECT,
			'options'     => [
				'icon'   => esc_html__( 'Icon', 'billey' ),
				'signal' => esc_html__( 'Signal', 'billey' ),
			],
			'default'     => 'icon',
		] );

		$this->add_control( 'marker_icon', [
			'label'     => esc_html__( 'Marker Icon', 'billey' ),
		...
		$repeater->start_controls_tab( 'marker_content_tab', [
			'label' => esc_html__( 'Marker', 'billey' ),
		] );

		$repeater->add_control( 'address', [
			'label'       => esc_html__( 'Address or Coordinate', 'billey' ),
		...
		$repeater->start_controls_tab( 'marker_popup_tab', [
			'label' => esc_html__( 'Popup', 'billey' ),
		] );

		$repeater->add_control( 'overlay_display', [
			'label'   => esc_html__( 'Display', 'billey' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''       => esc_html__( 'Hover', 'billey' ),
				'always' => esc_html__( 'Always', 'billey' ),
			],
			'default' => '',
		] );
		...
		$repeater->add_control( 'image', [
			...
			'description' => esc_html__( 'Choose an image that displays on left info box', 'billey' ),
			'type'        => Controls_Manager::MEDIA,
			'classes'     => 'billey-control-media-auto',
		] );
		...
		$this->add_group_control( Group_Control_Image_Size::get_type(), [
			'label'     => esc_html__( 'Popup Image Size', 'billey' ),
	...
	private function add_map_markers_style_section() {
		$this->start_controls_section( 'markers_popup_style_section', [
			'label' => esc_html__( 'Markers Popup', 'billey' ),
		...
		$this->add_responsive_control( 'markers_popup_padding', [
			...
			'size_units' => [ 'px', '%', 'em' ],
			'selectors'  => [
				'{{WRAPPER}} .billey-map-overlay-info' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
			],
		] );
		...
		$this->add_responsive_control( 'markers_popup_width', [
			...
			'selectors'  => [
				'{{WRAPPER}} .billey-map-overlay-content' => 'width: {{SIZE}}{{UNIT}};',
			],
		] );
	...
	private function get_overlay_value( $settings, $marker, $position ) {
		...
		$this->add_render_attribute( $item_key, 'class', [
			'billey-map-overlay',
			"billey-map-overlay-{$marker['overlay_display']}",
		...
		if ( $marker['title'] !== '' || $marker['content'] !== '' ) {
			$info_template .= '<div class="billey-map-overlay-template"><div class="billey-map-overlay-content"><div class="billey-map-overlay-info">';

			if ( isset( $marker['image'] ) ) {
				$size  = \Billey_Image::elementor_parse_image_size( $settings, '80x9999', 'popup_image_size' );
	...
	private function get_map_style() {
		return [
			'no_label_bright_colors'  => esc_html__( 'No Label Bright Colors', 'billey' ),
			'grayscale'               => esc_html__( 'Grayscale', 'billey' ),
			'subtle_grayscale'        => esc_html__( 'Subtle Grayscale', 'billey' ),
			'apple_paps_esque'        => esc_html__( 'Apple Maps-esque', 'billey' ),
			'pale_dawn'               => esc_html__( 'Pale Dawn', 'billey' ),
			'midnight_commander'      => esc_html__( 'Midnight Commander', 'billey' ),
			'blue_water'              => esc_html__( 'Blue Water', 'billey' ),
			'retro'                   => esc_html__( 'Retro', 'billey' ),
			'paper'                   => esc_html__( 'Paper', 'billey' ),
			'ultra_light_with_labels' => esc_html__( 'Ultra Light with Labels', 'billey' ),
			'shades_of_grey'          => esc_html__( 'Shades Of Grey', 'billey' ),
			'easy_light'              => esc_html__( 'Easy Light', 'billey' ),
			'wy'                      => esc_html__( 'WY', 'billey' ),
		];
	}
	...
	private function get_default_marker_icon() {
		$icon_url = BILLEY_ELEMENTOR_ASSETS . '/images/map-marker.png';
