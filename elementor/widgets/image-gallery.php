...
class Widget_Image_Gallery extends Base {
	...
	public function get_title() {
		return esc_html__( 'Image Gallery', 'billey' );
	}
	...
	private function add_content_section() {
		...
		$this->add_control( 'gallery', [
			'label'      => esc_html__( 'Add Images', 'billey' ),
	...
	private function add_layout_section() {
		...
		$this->add_control( 'lightbox_enable', [
			'label'        => esc_html__( 'Lightbox', 'billey' ),
			'type'         => Controls_Manager::SWITCHER,
			'return_value' => '1',
		] );
	...
	protected function render() {
		...
		$this->add_render_attribute( 'grid_wrapper', 'class', 'billey-grid-wrapper' );
		...
		if ( ! empty( $settings['lightbox_enable'] ) ) {
			$this->add_render_attribute( 'grid_wrapper', 'class', 'billey-light-gallery' );
		}
		?>
	...
	private function print_image( $image, $image_size ) {
		...
		if ( ! empty( $settings['lightbox_enable'] ) ) {
			...
			$image_full = \Billey_Image::get_attachment_info( $image['id'] );
