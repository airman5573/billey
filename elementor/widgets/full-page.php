...
class Widget_Full_Page extends Base {
	...
	public function __construct($data = [], $args = null) {
		...
		wp_register_style( 'billey-widget-fullpage', BILLEY_THEME_URI . '/assets/libs/fullpage/fullpage.min.css', null, '3.0.5' );

		// Fullpage.
		wp_register_script( 'fullpage', BILLEY_THEME_URI . '/assets/libs/fullpage/fullpage.js', array(), null, true );
		//wp_register_script( 'fullpage-extensions', BILLEY_THEME_URI . '/assets/libs/fullpage/fullpage.extensions.min.js', array(), null, true );
		wp_register_script( 'scrolloverflow', BILLEY_THEME_URI . '/assets/libs/fullpage/vendors/scrolloverflow.js', array(), null, true );
		wp_register_script( 'billey-widget-fullpage', BILLEY_ELEMENTOR_URI . '/assets/js/widgets/widget-fullpage.js', array(
			'elementor-frontend',
			'fullpage',
			'scrolloverflow',
		), null, true );
	...
	public function get_title() {
		return esc_html__( 'Fullpage', 'billey' );
	}
	...
	private function add_slides_section() {
		...
		$repeater->start_controls_tab( 'repeater_slides_content_tab', [ 'label' => esc_html__( 'Content', 'billey' ) ] );
		...
		$repeater->add_control( 'anchor', [
			'label' => esc_html__( 'Anchor', 'billey' ),
			'type'  => Controls_Manager::TEXT,
		] );

		$repeater->add_control( 'skin', [
			'label'       => esc_html__( 'Skin', 'billey' ),
			'description' => esc_html__( 'Controls the skin of pagination & header when view this section.', 'billey' ),
			'type'        => 'choose2',
			'options'     => [
				'light' => [
					'text' => esc_html__( 'Light', 'billey' ),
				],
				'dark'  => [
					'text' => esc_html__( 'Dark', 'billey' ),
				],
			],
			'toggle'      => false,
			'default'     => 'light',
		...
		$repeater->start_controls_tab( 'repeater_slides_background_tab', [ 'label' => esc_html__( 'Background', 'billey' ) ] );
		...
		$this->add_control( 'slides', [
			...
			'default'     => [
				[
					'title' => esc_html__( 'Slide #1', 'billey' ),
				],
				[
					'title' => esc_html__( 'Slide #2', 'billey' ),
				],
			],
			'title_field' => '{{{ title }}}',
		] );
	...
	private function add_fullpage_options_section() {
		$this->start_controls_section( 'fullpage_options_section', [
			'label' => esc_html__( 'Fullpage Options', 'billey' ),
		] );

		$this->add_control( 'dots', [
			'label' => esc_html__( 'Dots Enable', 'billey' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'dots_alignment', [
			'label'     => esc_html__( 'Dots Alignment', 'billey' ),
		...
		$this->add_control( 'numbers', [
			'label' => esc_html__( 'Numbers Enable', 'billey' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'sharing', [
			'label' => esc_html__( 'Share Enable', 'billey' ),
			'type'  => Controls_Manager::SWITCHER,
		] );
	...
	private function print_share( array $settings ) {
		...
		<div id="full-page-sharing" class="full-page-share-list">
			<div class="stalk">
				<?php esc_html_e( 'Stalk', 'billey' ); ?>
			</div>
			<?php \Billey_Templates::get_sharing_list( array(
	...
	private function print_slide_numbers( array $settings ) {
		...
		<div id="full-page-numbers" class="full-page-numbers">
			...
			<div class="title">
				<?php esc_html_e( 'Projects', 'billey' ); ?>
			</div>
		</div>
		<?php
	}
