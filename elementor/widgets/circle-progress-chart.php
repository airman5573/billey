...
class Widget_Circle_Progress_Chart extends Base {
	...
	public function get_title() {
		return esc_html__( 'Circle Progress Chart', 'billey' );
	}
	...
	private function add_chart_section() {
		...
		$this->add_control( 'size', [
			'label'       => esc_html__( 'Circle Size', 'billey' ),
			'description' => esc_html__( 'Controls the size of the pie chart circle.', 'billey' ),
			'type'        => Controls_Manager::NUMBER,
			'default'     => 180,
			'min'         => 100,
			'max'         => 1000,
		...
		$this->add_control( 'unit', [
			'label'       => esc_html__( 'Measuring unit', 'billey' ),
			'description' => esc_html__( 'Controls the unit of chart.', 'billey' ),
			'type'        => Controls_Manager::TEXT,
			'default'     => '%',
		] );

		$this->add_control( 'reverse', [
			'label' => esc_html__( 'Reverse animation and arc draw', 'billey' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'inner_content_type', [
			'label'   => esc_html__( 'Inner Content', 'billey' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				''     => esc_html__( 'Animate Number', 'billey' ),
				'text' => esc_html__( 'Custom Text', 'billey' ),
			],
			'default' => '',
		] );
	...
	private function add_chart_style_section() {
		...
		$this->add_control( 'bar_heading', [
			'label'     => esc_html__( 'Bar', 'billey' ),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		] );

		$this->add_control( 'line_cap', [
			'label'   => esc_html__( 'Line Cap', 'billey' ),
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'butt'   => esc_html__( 'Butt', 'billey' ),
				'round'  => esc_html__( 'Round', 'billey' ),
		...
		$this->add_control( 'line_width', [
			'label'   => esc_html__( 'Line Width', 'billey' ),
			'type'    => Controls_Manager::NUMBER,
			'default' => 3,
			'min'     => 1,
			'max'     => 50,
		...
		$this->add_control( 'bar_color_type', [
			'label'       => esc_html__( 'Color Type', 'billey' ),
			'type'        => Controls_Manager::CHOOSE,
			'label_block' => false,
			'toggle'      => false,
			...
			'options'     => [
				'classic'  => [
					'title' => esc_html__( 'Classic', 'billey' ),
					'icon'  => 'eicon-paint-brush',
				],
				'gradient' => [
					'title' => esc_html__( 'Gradient', 'billey' ),
					'icon'  => 'eicon-barcode',
				],
			],
		] );
		...
		$this->add_control( 'bar_color_2', [
			'label'     => esc_html__( 'Second Color', 'billey' ),
			'type'      => Controls_Manager::COLOR,
			'condition' => [
		...
		$this->add_control( 'track_heading', [
			'label'     => esc_html__( 'Track', 'billey' ),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		] );
	...
	protected function render() {
		...
		$bar_color_1 = ! empty( $settings['bar_color'] ) ? $settings['bar_color'] : '#111111';
		$bar_color_2 = ! empty( $settings['bar_color_2'] ) ? $settings['bar_color_2'] : '#111111';

		switch ( $settings['bar_color_type'] ) {
			case 'gradient' :
				$bar_color = '{"gradient": ["' . $bar_color_1 . '", "' . $bar_color_2 . '"]}';
				break;
			default;
				$bar_color = '{ "color": "' . $bar_color_1 . '" }';
				break;
		}
