...
abstract class Carousel_Base extends Base {
	...
	const SLIDER_WRAPPER_KEY = 'wrapper';

	public function get_script_depends() {
		return [ 'billey-group-widget-carousel' ];
	}
	...
	protected function get_slider_settings( array $settings ) {
		...
		if ( ! empty( $settings['swiper_dots_show'] ) ) {
			...
			if ( in_array( $settings['swiper_dots_style'], array( '03', '04', '06', '07' ) ) ) {
		...
		if ( ! empty( $settings['swiper_auto_height'] ) && 'yes' === $settings['swiper_auto_height'] ) {
			$slider_settings['data-auto-height'] = '1';
		}
		...
		if ( ! empty( $settings['swiper_pause_on_hover'] ) && 'yes' === $settings['swiper_pause_on_hover'] ) {
	...
	protected function get_slider_key() {
		return self::SLIDER_KEY;
	}

	protected function get_slider_wrapper_key() {
		return self::SLIDER_WRAPPER_KEY;
	}
	...
	protected function print_slider( array $settings = null ) {
		...
		<div <?php $this->print_render_attribute_string( self::SLIDER_KEY ); ?>>
			<div class="swiper-inner">

				<?php $this->before_swiper_container(); ?>
				...
				<?php $this->after_swiper_container(); ?>
	...
	protected function add_swiper_options_section() {
		$this->start_controls_section( 'swiper_options_section', [
			'label' => esc_html__( 'Carousel Options', 'billey' ),
		] );

		$this->add_responsive_control( 'swiper_items', [
			'label'          => esc_html__( 'Slides Per View', 'billey' ),
			'type'           => Controls_Manager::SELECT,
			'options'        => array(
				'auto'       => esc_html__( 'Auto', 'billey' ),
				'auto-fixed' => esc_html__( 'Auto - Fixed Width', 'billey' ),
				'1'          => '1',
				'2'          => '2',
				'3'          => '3',
				'4'          => '4',
		...
		$this->add_responsive_control( 'swiper_gutter', [
			'label'   => esc_html__( 'Space Between', 'billey' ),
		...
		$this->add_control( 'swiper_effect', [
			'label'   => esc_html__( 'Transition', 'billey' ),
			'type'    => Controls_Manager::SELECT,
			'options' => array(
				'slide' => esc_html__( 'Slide', 'billey' ),
				'fade'  => esc_html__( 'Fade', 'billey' ),
			),
			'default' => 'slide',
		] );
		...
		$this->add_control( 'swiper_autoplay', [
			...
			'description' => esc_html__( 'Delay between transitions (in ms). For e.g: 3000. Leave blank to disabled. Input 1 to make smooth transition.', 'billey' ),
			'type'        => Controls_Manager::NUMBER,
			'default'     => '',
		] );

		$this->add_control( 'swiper_pause_on_hover', [
			'label'   => esc_html__( 'Pause on Hover', 'billey' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );

		$this->add_control( 'swiper_loop', [
			'label'   => esc_html__( 'Infinite Loop', 'billey' ),
			'type'    => Controls_Manager::SWITCHER,
			'default' => 'yes',
		] );

		$this->add_control( 'swiper_centered', [
			'label' => esc_html__( 'Centered', 'billey' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'swiper_free_mode', [
			'label' => esc_html__( 'Free Mode', 'billey' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'swiper_auto_height', [
			'label' => esc_html__( 'Auto Height', 'billey' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'swiper_touch', [
			'label'       => esc_html__( 'Touchable', 'billey' ),
			'description' => esc_html__( 'Click and drag to change slides', 'billey' ),
			'type'        => Controls_Manager::SWITCHER,
			'default'     => 'yes',
		] );

		$this->add_control( 'swiper_mousewheel', [
			'label' => esc_html__( 'Mousewheel', 'billey' ),
			'type'  => Controls_Manager::SWITCHER,
		] );

		$this->add_control( 'swiper_navigation_heading', [
			'label'     => esc_html__( 'Navigation', 'billey' ),
		...
		$this->add_control( 'swiper_inner_heading', [
			'label'     => esc_html__( 'Slider Inner', 'billey' ),
			'type'      => Controls_Manager::HEADING,
			'separator' => 'before',
		] );
	...
	private function add_swiper_arrows_popover() {
		$this->add_control( 'swiper_arrows_show', [
			'label'        => esc_html__( 'Arrows', 'billey' ),
		...
		$this->add_control( 'swiper_arrows_aligned_by', [
			'label'       => esc_html__( 'Aligned By', 'billey' ),
			'type'        => Controls_Manager::SELECT,
			'options'     => [
		...
		$this->add_control( 'swiper_left_arrow_heading', [
			'label' => esc_html__( 'Left Arrow', 'billey' ),
			'type'  => Controls_Manager::HEADING,
		] );
		...
		$this->add_control( 'swiper_right_arrow_heading', [
			'label' => esc_html__( 'Right Arrow', 'billey' ),
			'type'  => Controls_Manager::HEADING,
		] );
		...
		$this->add_control( 'swiper_arrows_visibility_heading', [
			'label' => esc_html__( 'Visibility', 'billey' ),
			'type'  => Controls_Manager::HEADING,
		] );

		$this->add_control( 'swiper_arrows_show_always', [
			'label' => esc_html__( 'Show Always', 'billey' ),
			'type'  => Controls_Manager::SWITCHER,
		] );
	...
	private function add_swiper_dots_popover() {
		...
		$this->add_control( 'swiper_dots_style', [
			...
			'type'    => Controls_Manager::SELECT,
			'options' => [
				'01' => esc_html__( '01 - Circle Bullets', 'billey' ),
				'02' => esc_html__( '02 - Rectangle Bullets', 'billey' ),
				'03' => esc_html__( '03 - Fraction', 'billey' ),
				'04' => esc_html__( '04 - Circle Fraction & Arrows', 'billey' ),
				'05' => esc_html__( '05 - Modern Circle Bullets', 'billey' ),
				'06' => esc_html__( '06 - Progress Bar & Fraction & Right Arrow', 'billey' ),
				'07' => esc_html__( '07 - Fraction Style 02', 'billey' ),
			],
			'default' => '01',
		] );
		...
		$this->add_responsive_control( 'swiper_dots_vertical_align', [
			...
			'options'              => [
				...
				'below'  => [
					'title' => esc_html__( 'Below Slider', 'billey' ),
		...
		$this->add_responsive_control( 'swiper_dots_vertical_offset', [
			'label'       => esc_html__( 'Vertical Offset', 'billey' ),
			'type'        => Controls_Manager::SLIDER,
			'size_units'  => [ 'px', '%' ],
			'range'       => [
				'px' => [
		...
		$this->add_responsive_control( 'swiper_dots_horizontal_offset', [
			'label'       => esc_html__( 'Horizontal Offset', 'billey' ),
			'type'        => Controls_Manager::SLIDER,
			'size_units'  => [ 'px', '%' ],
			'range'       => [
				'px' => [
	...
	protected function add_swiper_arrows_style_section() {
		$this->start_controls_section( 'swiper_arrows_style_section', [
			'label'     => esc_html__( 'Carousel Arrows', 'billey' ),
	...
	protected function add_swiper_dots_style_section() {
		$this->start_controls_section( 'swiper_dots_style_section', [
			'label'     => esc_html__( 'Carousel Dots', 'billey' ),
			'tab'       => Controls_Manager::TAB_STYLE,
			'condition' => [
		...
		$this->add_control( 'swiper_dots_primary_color', [
			...
			'selectors' => [
				...
				'{{WRAPPER}} .progressbar .filled'                                       => 'background: {{VALUE}};',
				'{{WRAPPER}} .pagination-style-06 .fraction'                             => 'color: {{VALUE}};',
				'{{WRAPPER}} .pagination-style-06 .swiper-pagination .fraction .current' => 'color: {{VALUE}};',
				'{{WRAPPER}} .pagination-style-06 .swiper-pagination-inner'              => 'color: {{VALUE}};',
			],
		] );
	...
	protected function after_swiper_container() {
	}

	protected function before_render_slider_wrapper() {
	}

	protected function after_render_slider_wrapper() {
	}

	protected function before_render_slider() {
	}

	protected function after_render_slider() {
	}

	protected function render() {
		$settings = $this->get_settings_for_display();
		$this->add_render_attribute( $this->get_slider_wrapper_key(), 'class', 'billey-swiper-wrapper' );

		$this->before_render_slider_wrapper();
		?>
		<div <?php $this->print_render_attribute_string( self::SLIDER_WRAPPER_KEY ); ?>>
			<?php
			$this->before_render_slider();

			$this->print_slider( $settings );

			$this->after_render_slider();
			?>
		</div>
		<?php
		$this->after_render_slider_wrapper();
	}
}
