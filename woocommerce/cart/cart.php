			...
			if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
				...
				<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
					...
					<td class="product-name" data-title="<?php esc_attr_e( 'Product', 'billey' ); ?>">
						...
						// Billey Edited Code.
						if ( ! $product_permalink ) {
							$product_name = sprintf( '<h6 class="product-title">%s</h6>', esc_html( $_product->get_name() ) );
						} else {
							$product_name = sprintf( '<h6 class="product-title"><a href="%s">%s</a></h6>', esc_url( $product_permalink ), esc_html( $_product->get_name() ) );
						}
						$product_name = apply_filters( 'woocommerce_cart_item_name', $product_name, $cart_item, $cart_item_key );
						echo wp_kses( $product_name, 'billey-product-name' );
						...
						if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
							echo wp_kses( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'billey' ) . '</p>' ), [
								'p' => [
									'class' => [],
								],
							] );
					...
					<td class="product-price" data-title="<?php esc_attr_e( 'Price', 'billey' ); ?>">
						<?php
						echo apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key );
						?>
					</td>

					<td class="product-quantity" data-title="<?php esc_attr_e( 'Quantity', 'billey' ); ?>">
						<?php
					...
					<td class="product-subtotal" data-title="<?php esc_attr_e( 'Subtotal', 'billey' ); ?>">
						<label><?php esc_html_e( 'Subtotal :', 'billey' ); ?></label>
						<?php
					...
					<td class="product-remove">
						...
						echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
							...
							esc_attr( 'tm-button style-text' ),
							esc_attr__( 'Remove this item', 'billey' ),
							esc_attr( $product_id ),
							esc_attr( $_product->get_sku() ),
							esc_html__( 'Remove', 'billey' )
						), $cart_item_key );
						?>
					</td>
				</tr>
				...
				<div class="actions-wrap">
					<?php if ( wc_coupons_enabled() ) { ?>
						<div class="coupon-wrap">
							<div class="coupon">
								<label for="coupon_code"><?php esc_html_e( 'Coupon:', 'billey' ); ?></label>
